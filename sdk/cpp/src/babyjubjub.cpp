...
void jubjub_point::assert_equal(jubjub_point& p, jubjub_point& q) {
    bn254fr_class::assert_equal(p.x, q.x);
    bn254fr_class::assert_equal(p.y, q.y);
}
...
{
    jubjub_point r;
    ligetron::mux(r.x, cond, b0.x, b1.x);
    ligetron::mux(r.y, cond, b0.y, b1.y);
    return r;
}
...
void jubjub_point_vec::assert_equal(const jubjub_point_vec& p, const jubjub_point_vec& q) {
    vbn254fr_class::assert_equal(p.x, q.x);
    vbn254fr_class::assert_equal(p.y, q.y);
}

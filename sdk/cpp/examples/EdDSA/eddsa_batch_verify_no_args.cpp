...
vbn254fr_class message = 42;

// Private key = 114514
jubjub_point_vec public_key {
...
int main() {
    vbn254fr_class digest;

    auto *ctx = poseidon2_vbn254_context_new();
    poseidon2_vbn254_digest_init(ctx);
    poseidon2_vbn254_digest_update(ctx, signature.R.x);
    poseidon2_vbn254_digest_update(ctx, signature.R.y);
    poseidon2_vbn254_digest_update(ctx, public_key.x);
    poseidon2_vbn254_digest_update(ctx, public_key.y);
    poseidon2_vbn254_digest_update(ctx, message);

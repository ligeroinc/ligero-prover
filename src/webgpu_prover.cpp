...
int main(int argc, const char *argv[]) {
    ...
    if (jconfig.contains("args")) {
        for (const auto& arg : jconfig["args"]) {
            ...
            if (arg.contains("i64")) {
                ...
                std::cerr << "Error: Invalid args type: " << arg.dump()
                          << std::endl;
                exit(EXIT_FAILURE);
            }
        }
    ...
    // ------------------------------------------------------------
    params::hasher::digest program_hash;
    std::unique_ptr<wabt::Module> wabt_module{new wabt::Module{}};
    {
        ...
        program_hash = zkp::hash<params::hasher>(
            zkp::byte_view{program_data.data(), program_data.size()});
    }
    ...
    auto ctx =
        std::make_unique<zkp::nonbatch_stage1_context<field_t,
                                                      executor_t,
    ...
    std::copy(stage1_seed.begin(), stage1_seed.end(), seed);
    ...
    auto ctx2 = std::make_unique<
        zkp::nonbatch_stage2_context<field_t,
                                     executor_t,
                                     zkp::stage2_random_policy>>(executor);
    ...
    auto encoded_code_limbs =
        executor.template copy_to_host<uint32_t>(code_poly);
    encoded_code.import_limbs(encoded_code_limbs.data(),
                              encoded_code_limbs.size(),
                              sizeof(uint32_t),
                              field_t::num_u32_limbs);

    auto encoded_linear_limbs =
        executor.template copy_to_host<uint32_t>(linear_poly);
    encoded_linear.import_limbs(encoded_linear_limbs.data(),
                                encoded_linear_limbs.size(),
                                sizeof(uint32_t),
                                field_t::num_u32_limbs);

    auto encoded_quad_limbs =
        executor.template copy_to_host<uint32_t>(quad_poly);
    encoded_quad.import_limbs(encoded_quad_limbs.data(),
                              encoded_quad_limbs.size(),
                              sizeof(uint32_t),
                              field_t::num_u32_limbs);

    auto stage2_seed = zkp::hash<params::hasher>("LigetronStage2",
                                                 stage1_root,
                                                 encoded_code_limbs,
    ...
    std::vector<uint32_t> code_limbs =
        executor.template copy_to_host<uint32_t>(code_poly);
    ...
    std::vector<uint32_t> linear_limbs =
        executor.template copy_to_host<uint32_t>(linear_poly);
    ...
    std::vector<uint32_t> quad_limbs =
        executor.template copy_to_host<uint32_t>(quad_poly);
...

    {
        ...
        auto ctx3 = std::make_unique<
            ...
            executor, sample_index);
        ctx3->init_encoding_random(encoding_random_seed, params::any_iv);

        run_program(*wabt_module, *ctx3, input_args, indices_set);

        ligero::v1::ProofMetadata metadata;
        metadata.set_prover_version(std::format("{}.{}.{}",
                                                LIGETRON_VERSION_MAJOR,
                                                LIGETRON_VERSION_MINOR,
                                                LIGETRON_VERSION_PATCH));
        metadata.set_proof_schema_version(1);
        metadata.set_proof_type(ligero::common::v1::PROOF_TYPE_CLASSIC);
        zkp::digest_to_proto(metadata.mutable_program_hash(), program_hash);
        metadata.set_packing_size(static_cast<uint32_t>(k));
        metadata.set_codeword_size(static_cast<uint32_t>(n));
        metadata.set_sample_size(static_cast<uint32_t>(params::sample_size));
        metadata.set_security_level(128);
        {
            auto now  = std::chrono::system_clock::now();
            auto secs = std::chrono::duration_cast<std::chrono::seconds>(
                now.time_since_epoch());
            metadata.mutable_generated_at()->set_seconds(secs.count());
        }

        std::string serialized = zkp::serialize_proof(stage1_root,
                                                      encoded_code_limbs,
                                                      encoded_linear_limbs,
                                                      encoded_quad_limbs,
                                                      decommit,
        ...
        std::string compressed_proof;
        {
            io::filtering_ostream gzip_out;
            gzip_out.push(
                io::gzip_compressor(io::gzip_params(gzip_compression_level)));
            gzip_out.push(io::back_inserter(compressed_proof));
            gzip_out.write(serialized.data(), serialized.size());
        }

        std::ofstream proof_file(
            proof_name, std::ios::out | std::ios::binary | std::ios::trunc);

        if (!proof_file) {
            std::cerr << std::format("Error: Could not write to file \"{}\"",
                                     proof_name)
                      << std::endl;
            exit(EXIT_FAILURE);
        }

        proof_file.write(compressed_proof.data(), compressed_proof.size());
        proof_file.close();
    }
    ...
    bool valid_code   = std::all_of(host_code.begin() + k,
                                  host_code.end(),
    ...
    std::cout << "------------------------------------------" << std::endl;
    std::cout << "Final prove result:                  " << prove_result
              << std::endl;
